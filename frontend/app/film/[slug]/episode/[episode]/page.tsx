...
export default function EpisodeWatchPage() {
  ...
  const seasonNum = episodeMatch ? parseInt(episodeMatch[1]) : null;
  const episodeNum = episodeMatch ? parseInt(episodeMatch[2]) : null;
  ...
  useEffect(() => {
    const fetchEpisodeServers = async () => {
      ...
      try {
        ...
        setError('Gagal memuat server streaming');
      } finally {
        setIsLoading(false);
      }
    };
  ...
  return (
    <div className="container mx-auto px-4 py-6">
      ...
      <nav className="mb-4 text-sm text-gray-400">
        ...
        <span className="mx-2">/</span>
        <Link href={`/film/${seriesSlug}`} className="hover:text-primary">Series</Link>
        <span className="mx-2">/</span>
        <Link href={`/film/${seriesSlug}/episodes`} className="hover:text-primary">Episodes</Link>
        <span className="mx-2">/</span>
        <span className="text-white">Season {seasonNum} Episode {episodeNum}</span>
      </nav>
      ...
      {servers.length > 0 ? (
        ...
        <div className="bg-gray-800/50 rounded-xl p-8 text-center mb-8">
          ...
          <p className="text-gray-500 mt-2">Silakan coba episode lain atau kembali lagi nanti</p>
        </div>
      )}
      ...
      <div className="bg-gradient-to-r from-blue-900/30 to-purple-900/30 border border-blue-500/30 rounded-xl p-5 mb-8">
        <div className="flex items-start gap-4">
          ...
          <div>
            <h4 className="text-white font-semibold mb-2">Tips Nonton</h4>
