...
function SearchContent() {
  ...
  const fetchData = useCallback(async () => {
    ...
    setError(null);
    try {
      const result = await animeApi.search(query, page);
  ...
  const handleSearch = (e: React.FormEvent) => {
    e.preventDefault();
    if (searchInput.trim()) {
      window.location.href = `/anime/search?q=${encodeURIComponent(searchInput.trim())}`;
    }
  };
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      {!query ? (
        ...
        <div className="text-center py-20">
          <p className="text-gray-500">Tidak ada anime ditemukan untuk &quot;{query}&quot;</p>
        </div>
      )}
    </div>
  );
