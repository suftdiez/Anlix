...
import { FiArrowLeft, FiBook, FiTag } from 'react-icons/fi';
...
export default function TagDetailPage() {
  ...
  useEffect(() => {
    const fetchNovels = async () => {
      setIsLoading(true);
      try {
        const response = await novelApi.getByTag(tagSlug, page);
        if (response.success) {
    ...
    if (tagSlug) fetchNovels();
  }, [tagSlug, page]);

  return (
    <div className="container mx-auto px-4 py-6">
      {/* Back Button */}
      <Link href="/novel/tag" className="inline-flex items-center gap-2 text-gray-400 hover:text-white mb-6 transition">
      ...
      {novels.length === 0 && !isLoading && (
        <div className="text-center py-12">
          ...
          <p className="text-gray-400">Tidak ada novel ditemukan dengan tag ini</p>
        </div>
      )}
