...
export default function TamatNovelPage() {
  ...
  useEffect(() => {
    const fetchNovels = async () => {
      setIsLoading(true);
      try {
        const response = await novelApi.getTamat(page);
        if (response.success) {
  ...
  return (
    <div className="container mx-auto px-4 py-6">
      <div className="mb-6">
        <h1 className="text-2xl md:text-3xl font-bold text-white mb-2">Novel Tamat</h1>
        <p className="text-gray-400 mb-4">Koleksi novel yang sudah selesai diterjemahkan</p>
      ...
      <div className="flex flex-wrap gap-2 mb-6">
        ...
        <Link href="/novel/tamat" className="px-4 py-2 bg-primary text-white rounded-lg font-medium">
          Tamat
        </Link>
      </div>
