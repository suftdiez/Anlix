...
import { FiSearch, FiBook, FiTrendingUp } from 'react-icons/fi';
...
export default function PopularNovelPage() {
  ...
  useEffect(() => {
    const fetchNovels = async () => {
      setIsLoading(true);
      try {
        const response = await novelApi.getPopular(page);
        if (response.success) {
  ...
  return (
    <div className="container mx-auto px-4 py-6">
      <div className="mb-6">
        <h1 className="text-2xl md:text-3xl font-bold text-white mb-2">Novel Populer</h1>
        <p className="text-gray-400 mb-4">Novel dengan pembaca terbanyak</p>
      ...
      <div className="flex flex-wrap gap-2 mb-6">
        ...
        <Link href="/novel/popular" className="px-4 py-2 bg-primary text-white rounded-lg font-medium">
          Populer
        </Link>
