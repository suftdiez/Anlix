...
export default function DonghuaRandomPage() {
  const router = useRouter();
  const [donghuas, setDonghuas] = useState<DonghuaItem[]>([]);
  ...
  const fetchRandom = async () => {
    ...
    try {
      ...
      const result = await donghuaApi.getLatest(randomPage);
      const allItems = result.data || [];
      
      // Shuffle and pick 12 random items
      const shuffled = allItems.sort(() => Math.random() - 0.5);
      setDonghuas(shuffled.slice(0, 12));
    } catch (err) {
      console.error('Failed to fetch random donghua:', err);
  ...
  const handleSurpriseMe = () => {
    if (donghuas.length > 0) {
      const randomItem = donghuas[Math.floor(Math.random() * donghuas.length)];
      router.push(`/donghua/${randomItem.slug}`);
    }
  };
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      {isLoading ? (
        ...
        <motion.div 
          key={donghuas[0]?.id} // Re-animate on shuffle
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4"
        >
          {donghuas.map((item, index) => (
            <motion.div
              key={item.id}
        ...
        <div className="text-center py-20">
          ...
          <p className="text-gray-500">Tidak ada donghua ditemukan</p>
        </div>
      )}
    </div>
  );
