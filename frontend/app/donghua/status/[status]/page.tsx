...
export default function DonghuaStatusPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      setError(null);
      try {
        // Use status filter from API
        const result = await donghuaApi.getByStatus(status, page);
        setDonghuas(result.data || []);
        setHasNext(result.hasNext || false);
      } catch (err) {
        console.error('Failed to fetch donghua by status:', err);
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <p className="text-gray-400 mb-8">
        {isOngoing 
          ? 'Daftar donghua yang masih tayang dan update setiap minggu' 
          : 'Daftar donghua yang sudah selesai tayang'}
      </p>
      ...
      {isLoading ? (
        ...
        <div className="text-center py-20">
          ...
          <p className="text-gray-500">Tidak ada donghua {statusLabel.toLowerCase()} ditemukan</p>
        </div>
      )}
    </div>
  );
