...
export default function DonghuaDetailPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const data = await donghuaApi.getDetail(slug);
        setDonghua(data.data);

        if (isAuthenticated) {
          try {
            const bookmarkData = await userApi.checkBookmark(slug, 'donghua');
  ...
  if (!donghua) {
    return (
      <div className="container mx-auto px-4 py-20 text-center">
        <h1 className="text-2xl text-gray-400">Donghua tidak ditemukan</h1>
      </div>
    );
  }
  ...
  return (
    <div>
      ...
      <div className="relative h-[50vh] md:h-[60vh] overflow-hidden">
        <Image
          src={getImageUrl(donghua.poster)}
          alt={donghua.title}
          fill
        ...
        <div className="flex flex-col md:flex-row gap-8">
          ...
          >
            <div className="relative aspect-poster rounded-xl overflow-hidden shadow-2xl border border-white/10">
              <Image src={getImageUrl(donghua.poster)} alt={donghua.title} fill className="object-cover" />
            </div>
          </motion.div>
          ...
          <motion.div
            ...
            className="flex-1"
          >
            <span className="badge-accent mb-3">Donghua</span>
            <h1 className="text-3xl md:text-4xl font-display font-bold text-white mb-4">{donghua.title}</h1>

            <div className="flex flex-wrap items-center gap-4 mb-4 text-sm">
              {donghua.score && (
                <div className="flex items-center gap-1 text-accent">
                  <FiStar className="w-4 h-4 fill-current" />
                  <span className="font-semibold">{donghua.score}</span>
                </div>
              )}
              {donghua.status && (
              ...
              {donghua.totalEpisodes && (
                <div className="flex items-center gap-1 text-gray-400">
                  ...
                  <span>{donghua.totalEpisodes} Episode</span>
                </div>
              )}
            </div>
            ...
            {donghua.genres && donghua.genres.length > 0 && (
              <div className="flex flex-wrap gap-2 mb-6">
                {donghua.genres.map((genre) => (
                  <Link key={genre} href={`/genre/${genre.toLowerCase()}`} className="genre-tag">
                    {genre}
                  </Link>
                ))}
              </div>
            ...
            <div className="flex items-center gap-4">
              {donghua.episodes.length > 0 && (
                <Link href={`/donghua/${slug}/${donghua.episodes[0].slug}`} className="btn-primary flex items-center gap-2">
        ...
        >
          ...
          {donghua.episodes.length > 0 ? (
            <div className="grid grid-cols-4 sm:grid-cols-6 md:grid-cols-8 lg:grid-cols-10 xl:grid-cols-12 gap-2">
              {donghua.episodes.map((episode) => (
                <Link
                  key={episode.id}
                  href={`/donghua/${slug}/${episode.slug}`}
