...
function SearchContent() {
  ...
  const fetchData = useCallback(async () => {
    ...
    setError(null);
    try {
      const result = await donghuaApi.search(query, page);
      setDonghuas(result.data || []);
      setHasNext(result.hasNext || false);
    } catch (err) {
      console.error('Failed to search donghua:', err);
  ...
  const handleSearch = (e: React.FormEvent) => {
    e.preventDefault();
    if (searchInput.trim()) {
      window.location.href = `/donghua/search?q=${encodeURIComponent(searchInput.trim())}`;
    }
  };
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      {!query ? (
        <div className="text-center py-20">
          ...
          <p className="text-gray-500">Masukkan kata kunci untuk mencari donghua</p>
        </div>
      ) : isLoading ? (
        ...
        <div className="text-center py-20">
          <p className="text-gray-500">Tidak ada donghua ditemukan untuk &quot;{query}&quot;</p>
        </div>
      )}
    </div>
  );
