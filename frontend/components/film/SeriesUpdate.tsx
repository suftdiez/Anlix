...
export default function SeriesUpdate() {
  ...
  useEffect(() => {
    const fetchSeriesUpdate = async () => {
      try {
        setIsLoading(true);
        const data = await filmApi.getSeriesUpdate();
        setSeries((data.data || []).slice(0, 12));
        setError(null);
      } catch (err) {
        console.error('Failed to fetch series update:', err);
        setError('Gagal memuat series update');
      } finally {
        setIsLoading(false);
      }
    };
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-3">
          <div className="p-2 bg-gradient-to-br from-green-500/20 to-emerald-500/20 rounded-lg">
            <FiRefreshCw className="w-5 h-5 text-green-400" />
          </div>
          <div>
            ...
            <p className="text-sm text-gray-400">
              Series yang baru update episode terbaru
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={() => scroll('left')}
            className="p-2 bg-dark-card border border-white/10 rounded-lg text-gray-400 hover:text-white hover:border-green-500/50 transition-all"
